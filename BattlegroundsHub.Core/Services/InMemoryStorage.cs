    ...
    public static class InMemoryStorage
    {
        ...
        private static void InitializeRealData()
        {
            Heroes.AddRange(new List<Hero>
            {
                ...
                new Hero { Id = 19, Name = "Гафф Рунический Тотем", Title = "Герй", Tier = HeroTier.C, Description = "В конце хода случайный твой слуга получает +1/+2." },
            ...
            Minions.AddRange(new List<Minion>
            {
                ...
                new Minion { Id = 22, Name = "Пиратский капитан", TavernTier = 5, Types = new List<string> { "Пират" }, Attack = 5, Health = 4, Effect = "Даёт +1/+1 всем пиратов в бою." },
        ...
        {
            Console.WriteLine("=== ВАЛИДАЦИЯ НАЧАЛЬНЫХ ДАННЫХ ===");
            ...
            foreach (var hero in Heroes)
            {
                ...
                {
                    heroErrors++;
                    Console.WriteLine($"Ошибка в герое ID {hero.Id} '{hero.Name}': {string.Join(", ", errors)}");
                }
            }
            ...
            foreach (var minion in Minions)
            {
                ...
                {
                    minionErrors++;
                    Console.WriteLine($"Ошибка в миньоне ID {minion.Id} '{minion.Name}': {string.Join(", ", errors)}");
                }
            }

            if (heroErrors == 0 && minionErrors == 0)
            {
                Console.WriteLine("✅ Все начальные данные прошли валидацию");
            }
            else
            {
                Console.WriteLine($"⚠️ Найдено ошибок: герои - {heroErrors}, миньоны - {minionErrors}");
            }
            Console.WriteLine();
        }
        ...
        public static (bool Success, List<string> Errors) AddHeroWithValidation(Hero hero)
        {
            var errors = HeroValidator.Validate(hero);
            if (errors.Count > 0)
            {
        ...
        public static (bool Success, List<string> Errors) AddMinionWithValidation(Minion minion)
        {
        ...
        public static (bool Success, List<string> Errors, int ValidHeroes, int ValidMinions)
            ReplaceAllWithValidation(List<Hero> newHeroes, List<Minion> newMinions)
        {
            newHeroes ??= new List<Hero>();
            newMinions ??= new List<Minion>();
            ...
            for (int i = 0; i < newHeroes.Count; i++)
            {
                ...
                var heroErrors = HeroValidator.Validate(hero);
                if (heroErrors.Count > 0)
                {
                    allErrors.Add($"Герой #{i + 1} '{hero.Name}': {string.Join("; ", heroErrors)}");
                }
                else
                {
                    validHeroes++;
            ...
            for (int i = 0; i < newMinions.Count; i++)
            {
                var minion = newMinions[i];
                var minionErrors = MinionValidator.Validate(minion);
                if (minionErrors.Count > 0)
                {
                    allErrors.Add($"Миньон #{i + 1} '{minion.Name}': {string.Join("; ", minionErrors)}");
                }
                else
                {
                    validMinions++;
            ...
            {
                return (false, allErrors, validHeroes, validMinions);
            }
            ...
            Minions.AddRange(newMinions);

            _nextHeroId = Heroes.Count == 0 ? 1 : Heroes.Max(h => h.Id) + 1;
            _nextMinionId = Minions.Count == 0 ? 1 : Minions.Max(m => m.Id) + 1;

            return (true, new List<string>(), validHeroes, validMinions);
        }

        public static void ReplaceAll(List<Hero> newHeroes, List<Minion> newMinions)
        {
            newHeroes ??= new List<Hero>();
            newMinions ??= new List<Minion>();
        ...
        public static (bool Success, List<string> Errors) UpdateHeroWithValidation(int heroId,
            string newName, string newTitle, HeroTier newTier, string newDescription)
        {
            var hero = FindHeroById(heroId);
            if (hero == null)
            {
                return (false, new List<string> { $"Герой с ID {heroId} не найден" });
            }
            ...
            var errors = HeroValidator.Validate(tempHero);
            if (errors.Count > 0)
            {
                return (false, errors);
            }
            ...
            hero.Title = newTitle?.Trim() ?? "";
            hero.Tier = newTier;
            hero.Description = newDescription?.Trim() ?? "";

            return (true, new List<string>());
        }

        public static (bool Success, List<string> Errors) UpdateMinionWithValidation(int minionId,
            string newName, int newTavernTier, List<string> newTypes, int newAttack,
            int newHealth, string newEffect)
        {
            var minion = FindMinionById(minionId);
            if (minion == null)
            {
                return (false, new List<string> { $"Миньон с ID {minionId} не найден" });
            }
            ...
            var tempMinion = new Minion
            {
                ...
                Types = newTypes ?? new List<string>(),
                Attack = newAttack,
                Health = newHealth,
                Effect = newEffect
            };

            var errors = MinionValidator.Validate(tempMinion);
            if (errors.Count > 0)
            {
                return (false, errors);
            }

            minion.Name = newName.Trim();
            minion.TavernTier = newTavernTier;
            minion.Types = newTypes ?? new List<string>();
            minion.Attack = newAttack;
            minion.Health = newHealth;
            minion.Effect = newEffect?.Trim() ?? "";
